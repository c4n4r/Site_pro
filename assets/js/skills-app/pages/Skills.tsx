    ...
    render(){
        ...
        return (
           <div className="h-screen w-full flex flex-col mt-12 p-12 text-light ">
               ...
               <select onChange={this.selectCategory} className="h-12 text-darken w-1/12 border-golden focus:outline-none focus:border-1 focus:border-golden" name="categories" id="categories">
                   {this.state.categories.map((category: Category) => (
                       <option  className="px-4 text-center" key={category.id} value={category.id}>{category.name}</option>
                   ))}
               </select>

               <div className="w-5/6 flex flex-row self-center border-b border-golden my-16" />

               <div className="flex flex-row justify-around w-full my-6">
                   {this.state.technos.map((techno:Techno) => {
                       return (<TechnoCard key={techno.id} clicked={this.selectTechno} selectedTechno={techno}/>)
                   })}
               </div>

               {this.state.skills.length > 0 ? <div className="w-4/6 flex flex-row self-center border-b border-golden my-24"/>: <div/>}


               <div className="flex flex-row justify-around w-2/3 self-center  my-10">
                   {this.state.skills.map((skill:Skill) => {
                       return (<SkillCard hover={this.skillHover} mouseOut={this.skillOut} key={skill.id} selectedSkill={skill}/>)
                   })}
               </div>


               { selectedSkill?
                   <div className="mt-12 w-1/3 p-12 text-golden flex flew-row self-center text-center items-center justify-center border border-golden rounded-lg"><p className="m-6">{this.state.selectedSkill.description}</p></div> :
                   <div/>
               }
